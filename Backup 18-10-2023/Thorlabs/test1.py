...
class MotorizedStageApp(tk.Tk):
    def __init__(self):
        ...
        self.title("Motorized Stage Control")
    ...
    def step_length_popup(self, axis):
        popup = tk.Toplevel(self)
        popup.title(f"{axis.upper()}-Axis Step Length")

        label = tk.Label(popup, text=f"Enter step length for {axis}-axis:")
        label.pack(side="top", padx=10, pady=10)

        entry = tk.Entry(popup)
        entry.pack(side="top", padx=10, pady=10)

        button = tk.Button(popup, text="OK", command=lambda: self.set_step_length(axis, entry.get(), popup))
        button.pack(side="top", padx=10, pady=10)


    def set_step_length(self, axis, step_length_str, popup):
        try:
            step_length = float(step_length_str)
            if step_length <= 0:
                raise ValueError
            if axis == "x":
                self.step_x = step_length
            elif axis == "y":
                self.step_y = step_length
            elif axis == "z":
                self.step_z = step_length
            popup.destroy()
        except ValueError:
            messagebox.showerror("Invalid input", "Please enter a positive number for the step length.")

    def move_x_up_continuous(self):
        while self.move_x_up_active:
            self.motor_x.move_by(self.step_x)
            time.sleep(0.2)
            while self.motor_x.is_in_motion:
                time.sleep(0.1)
    ...
    def move_y_up_continuous(self):
        while self.move_y_up_active:
            self.motor_y.move_by(self.step_y)
            time.sleep(0.2)
            while self.motor_y.is_in_motion:
                time.sleep(0.1)
    ...
    def move_z_up_continuous(self):
        while self.move_z_up_active:
            self.motor_z.move_by(self.step_z)
            time.sleep(0.2)
            while self.motor_z.is_in_motion:
                time.sleep(0.1)
