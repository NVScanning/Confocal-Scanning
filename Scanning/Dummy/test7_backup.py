...
class ScannerApp(tk.Tk):
    ...
    def create_control_panel(self):
        ...
        ttk.Label(scanning_control_frame, text="Integration Time").grid(row=3, column=2, padx=5, pady=5, sticky="w")
        ...
        self.start_button.grid(row=4, column=0, columnspan=1, padx=5, pady=5, sticky="ew")
        
        self.abort_button = ttk.Button(scanning_control_frame, text="Abort Scan", command=self.abort_scan)
        self.abort_button.grid(row=4, column=2, columnspan=1, padx=5, pady=5, sticky="ew")
    ...
    def scan_loop(self):
        ...
        for i in range(z_steps):
            for j in range(y_steps):
                ...
                num_samples = int(integration_time / 0.01)
                apd_signals = np.random.random(num_samples) + 1
                apd_signal = np.mean(apd_signals)
